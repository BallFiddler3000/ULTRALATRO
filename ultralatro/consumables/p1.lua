SMODS.Consumable {
    ...
    loc_txt = {
        name = 'P-1',
        text = {
        [1] = 'Gives a {C:purple}Flesh Prison{}'
    }
    },
    cost = 20,
    unlocked = true,
    ...
    atlas = 'CustomConsumables',use = function(self, card, area, copier)
        local used_card = copier or card
            ...
            G.E_MANAGER:add_event(Event({
                  ...
                  func = function()
                      play_sound('timpani')
                      local new_joker = SMODS.add_card({ set = 'Joker', key = 'j_ultra_fleshprison' })
                      if new_joker then
                      end
              ...
              delay(0.6)
            G.P_CENTERS["c_ultra_p1"].in_pool = function() return false end
    end,
